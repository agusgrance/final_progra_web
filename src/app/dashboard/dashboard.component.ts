...
export class DashboardComponent implements OnInit {
  ...
  onChangeCategory(e: any) {
    ...
    this.dataService.getBooksByList(this.list_name).subscribe({
      next: (response) => {
  ...
  loadCategory(list_name: string) {
    this.dataService.getBooksByList(list_name).subscribe({
      next: (response) => {
        this.booksList = response.results;
      },
      error: (e) => { },
  ...
  upload(angForm1: any) {
    ...
    this.dataService
      .postUpload(
        angForm1.value.comment,
        this.currentFile.name,
        memeDate,
        angForm1.value.isbn,
        angForm1.value.rating
      )
      .pipe(first())
      .subscribe(() => {
        this.listarCards();
  ...
  isSubmit(angForm: any, selectedFiles: any) {
    return (
      angForm.value.comment &&
      angForm.value.isbn &&
      angForm.value.rating &&
      selectedFiles
    );
  }
  ...
  ngOnInit(): void {
    ...
    this.listarComentarios();
    this.getBooks();
    this.loadCategory('combined-print-and-e-book-fiction');
    this.getUsers();
